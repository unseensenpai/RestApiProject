...
namespace TestProject.HttpApi.Employee
{
    public class EmployeeController : CoreController, IEmployeeService
    {
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<BaseDataResponse<IEnumerable<EmployeeResponseDto>>> GetEmployeeSalariesByDate([Required, FromBody] EmployeeParameters parameters)
            => await _employeeService.GetEmployeeSalariesByDate(parameters);
    }
}
